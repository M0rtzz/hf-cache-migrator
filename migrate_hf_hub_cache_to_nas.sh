...
# ===== 配置项：可以通过环境变量覆盖 =====
TARGET_HUB_CACHE="${TARGET_HUB_CACHE:-/nas/DO_NOT_EDIT_OR_DELETE_SHARED_CACHE/HuggingFace/hub}"
...
copy_hub_cache() {
  ...
  case "${COPY_BACKEND}" in
    rsync)
      # 使用 rsync 迁移；同名文件不覆盖，refs/lock/tmp/incomplete 不迁移。
      log "使用 rsync 迁移 Hugging Face hub cache。"
      if [[ "${COPY_LINKS_EFFECTIVE}" == "1" ]]; then
    ...
    fpsync)
      ...
      log "使用 fpsync 并行迁移 Hugging Face hub cache：jobs=${FPSYNC_JOBS}"
      if [[ "${COPY_LINKS_EFFECTIVE}" == "1" ]]; then
...
# 创建目标 Hugging Face hub cache 目录。
log "创建目标 Hugging Face hub cache 目录：${TARGET_HUB_CACHE}"
run mkdir -p "${TARGET_HUB_CACHE}"
...
if [[ "${COPY_LINKS}" == "yes" ]]; then
  ...
  symlink_test_target="${TARGET_HUB_CACHE}/.symlink-test-target-$$"
  symlink_test_link="${TARGET_HUB_CACHE}/.symlink-test-link-$$"
  log "检测目标文件系统是否支持 symlink：${TARGET_HUB_CACHE}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    log "DRY_RUN 模式下不实际检测 symlink，默认按支持 symlink 打印命令。"
  else
    printf 'test\n' > "${symlink_test_target}"
    if ln -s "$(basename "${symlink_test_target}")" "${symlink_test_link}" 2>/dev/null; then
      log "目标文件系统支持 symlink，将保留 Hugging Face cache 的 symlink 结构。"
...
echo "Target hub cache: ${TARGET_HUB_CACHE}"
echo "Group: ${GROUP}"
...
while IFS= read -r -d '' dir; do
  ...
  # 拼接用户本地 Hugging Face hub cache 目录。
  source_hub_cache="${dir}/.cache/huggingface/hub"

  # 如果用户没有本地 Hugging Face hub cache 目录，则跳过。
  if [[ ! -d "${source_hub_cache}" ]]; then
    log "用户没有本地 Hugging Face hub cache，跳过：${source_hub_cache}"
    skipped_entries+=("${dir} -> skipped: no local huggingface hub cache")
    continue
  fi

  # 迁移用户本地 Hugging Face hub cache 到 NAS；同名文件不覆盖，refs/lock/tmp/incomplete 不迁移。
  log "开始迁移用户 Hugging Face hub cache：user=${user}, source=${source_hub_cache}, target=${TARGET_HUB_CACHE}"
  copy_hub_cache "${source_hub_cache}" "${TARGET_HUB_CACHE}"

  # 记录已经迁移的用户。
  migrated_users+=("${user}")
done < <(find "${SCAN_ROOT}" -mindepth 1 -maxdepth 1 -type d -print0)

# 修正 NAS hub cache 的属主属组。
log "修正目标 hub cache 的属主属组：root:${GROUP}"
run chown -R "root:${GROUP}" "${TARGET_HUB_CACHE}"

# 修正 NAS hub cache 的基础权限：属主和属组可读写，其他用户无权限。
log "修正目标 hub cache 的基础权限：属主和属组可读写，其他用户无权限。"
run chmod -R u+rwX,g+rwX,o-rwx "${TARGET_HUB_CACHE}"

# 给 NAS hub cache 下所有目录设置 setgid，使后续新文件尽量继承共享组。
log "给目标 hub cache 下所有目录设置 setgid。"
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[dry-run] find '${TARGET_HUB_CACHE}' -type d -exec chmod 2770 {} +"
else
  find "${TARGET_HUB_CACHE}" -type d -exec chmod 2770 {} +
fi
